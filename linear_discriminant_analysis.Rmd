...
And fit the above model to the training data using the `lda()` function^[Note to my fellow New Englanders: the [MASS](https://cran.r-project.org/web/packages/MASS/index.html) R library that contains the `lda()` function is named for a book titled "Modern Applied Statistics with S", and has nothing to do with Massachusetts or the Boston dataset we're using here] (note: there is no `family` argument as with `glm()`, but the calls are otherwise identical):
